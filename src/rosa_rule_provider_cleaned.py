    ...
    def get_chair_score(self):
        ...
        if r_hip_knee_ankle_angle or l_hip_knee_ankle_angle:
            if r_hip_knee_ankle_angle:
                ...
                self.put_text_add_description(axis, self.output_path, self.file_name, 'Right leg angle',
                                              r_hip_knee_ankle_angle, c)
        ...
        if hasattr(self.pose_detector, 'Neck'):
            ...
            if shoulders_neck_angle:
                ...
                self.put_text_add_description(axis, self.output_path, self.file_name,
                                              "Not Enough Info aboud sholders being shrogged ", None, 'red', 10, 90)
        else:
            print("Not Enough Info")
            self.put_text_add_description(axis, self.output_path, self.file_name,
                                          "Not Enough Info aboud sholders being shrogged ", None, 'red', 10, 60)
            # import pudb; pu.db
        #########################################################################################
        status = 'vertical'
        if status == 'vertical':
            ...
            r_shoulder_elbow_points = [self.points[self.pose_detector.RElbow],
                                       self.points[self.pose_detector.RShoulder],
                                       [self.points[self.pose_detector.RShoulder][0],
                                        self.points[self.pose_detector.RShoulder][1] + 50]]

            l_shoulder_elbow_points = [self.points[self.pose_detector.LElbow],
                                       self.points[self.pose_detector.LShoulder],
                                       [self.points[self.pose_detector.LShoulder][0],
                                        self.points[self.pose_detector.LShoulder][1] + 50]]
            if r_shoulder_elbow_angle or l_shoulder_elbow_angle:
                ...
                if l_shoulder_elbow_angle is not None:
                    ...
                    self.put_text_add_description(axis, self.output_path, self.file_name, 'Left elbow angle',
                                                  l_shoulder_elbow_angle, c, 10, 90)

            else:

                print("Not Enough Info")
                self.put_text_add_description(axis, self.output_path, self.file_name,
                                              "Not Enough Info boud sholders being wide", None, 'red')
                armrest_score = None
        # import pudb; pu.db
        # plt.show()
